...
def saep():
    ...
    chb.grid(row = 1, column = 3)
    chb.bind('<Enter>', colorchanger)
    chb.bind('<Leave>', colorchanger)

    tk.Label(root_2, height = 20).grid(row = 4, column = 2)
    tk.Label(root_2, text = "© 2020 Copyright", font = (11)).grid(row = 4, column = 4, sticky = 's', pady = 60)
    tk.Label(root_2, text = "D:\\Advaith\\Code\\class\\pycode\\gamingconsole.py\nAll rights reserved\n version 1.1.3 full release 30-06-20", font = myfont).grid(row = 4, column = 4, sticky = 's')
root = tk.Tk()
